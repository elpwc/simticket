            ...
            <label className="ticket-form-label">
              ...
              <div className="flex grid-cols-3 gap-2">
                <label className="flex gap-1 items-center">
                  X
                  <input className="max-w-[50px]" type="number" value={drawParameters.offsetX} onChange={(e) => setDrawParameters((prev) => ({ ...prev, offsetX: Number(e.target.value) }))} />
                </label>
                <label className="flex gap-1 items-center">
                  Y
                  <input className="max-w-[50px]" type="number" value={0 - drawParameters.offsetY} onChange={(e) => setDrawParameters((prev) => ({ ...prev, offsetY: 0 - Number(e.target.value) }))} />
                </label>
                <button
            ...
            <PrettyInputRadioGroup
              ...
              value={
                CR_TRAIN_TYPE_ARRAY.includes(
                  drawParameters.routeIdentifier.substring(drawParameters.routeIdentifier.substring(0, 1) === '0' ? 1 : 0, drawParameters.routeIdentifier.substring(0, 1) === '0' ? 2 : 1)
                )
                  ? drawParameters.routeIdentifier.substring(drawParameters.routeIdentifier.substring(0, 1) === '0' ? 1 : 0, drawParameters.routeIdentifier.substring(0, 1) === '0' ? 2 : 1)
                  : ''
              }
              onChange={(value: string) => {
                ...
                if (CR_TRAIN_TYPE_ARRAY.includes(routeIdentifierFirstChar)) {
                  const routeIdentifierNumbersChar = drawParameters.routeIdentifier.substring(drawParameters.routeIdentifier.substring(0, 1) === '0' ? 2 : 1, drawParameters.routeIdentifier.length);
                          ...
                          {items.map((purchaseMethodItem) => {
                            return (
                              <label key={purchaseMethodItem.type + '_' + purchaseMethodItem.title} className="flex items-center gap-2">
                                <span className={purchaseMethodItem.title.length === 1 ? 'border-black flex justify-center items-center border rounded-[20px] text-[13px] w-[18px] h-[18px]' : ''}>
                            ...
                            <input
                              ...
                              onChange={() => {
                                if (isChecked) {
                                  setDrawParameters((prev) => ({
                                    ...prev,
                                    purchaseMethod: prev.purchaseMethod.filter((j) => !(j.type === purchaseMethodItem.type && j.title === purchaseMethodItem.title)),
                                  }));
                                } else {
