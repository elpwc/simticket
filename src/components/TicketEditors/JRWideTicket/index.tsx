...
export default function JRWideTicket() {
	...
	const [drawParameters, setDrawParameters] = useState<JRWideTicketDrawParameters>(() =>
		copyEditingTicketDataToDrawParameters && editingTicketData ? structuredClone(editingTicketData) : getInitialValues(),
	);
					...
					<EditorFormShell defaultSectionId="trip" sectionOrder={['trip', 'purchase', 'serial']}>
						<EditorFormShell.Section id="trip" label={t('editor.jr.mars.shell.trip')} description={t('editor.jr.mars.shell.tripDesc')}>
							...
							<TitleContainer title="券面" className="flex flex-wrap gap-1">
								<TicketFormLabel label={'券面'}>
									<JRWideTicketBgSelector
										is120mm={drawParameters.is120mm}
										value={drawParameters.background}
										onChange={(e) => setDrawParameters((prev) => ({ ...prev, background: e }))}
									/>
								</TicketFormLabel>
								<TicketFormLabel label={'券幅'}>
								...
								<div className="w-full">
									...
									<div>
										<Tab
											menuPosition="top"
											menu={JRTicketTypeList.map((JRTicketType) => ({
												title: JRTicketType.name,
											}))}
											pages={[
												<RegularForm
													onChange={(title) => {
								...
								<div className="ticket-form-label">
									...
									<div className="ticket-form-label__control">
										<Toggle
										...
										<Toggle
											...
											onChange={(value) => {
												setDrawParameters((prev) => ({
													...prev,
													hasCannotPassAutoPasiAreaMark: value,
												}));
											}}
										>
											改札利用不可マーク
								...
								<TicketFormLabel label={'タイトル下線様式'}>
									<PrettyInputRadioGroup
										value={drawParameters.titleUnderlineStyle}
										onChange={(value) => {
											setDrawParameters((prev) => ({
												...prev,
												titleUnderlineStyle: value,
											}));
										}}
							...
							<TitleContainer title="駅情報" className="flex flex-wrap gap-1">
								<div className="flex flex-col gap-[2px]">
									<TicketFormLabel label={'出発駅特定都区市内'}>
										<PrettyInputRadioGroup
											value={drawParameters.station1AreaChar}
											onChange={(value) => {
												setDrawParameters((prev) => ({
													...prev,
													station1AreaChar: value,
												}));
											}}
									...
									<TicketFormLabel label={<span className="font-bold">出発駅</span>}>
									...
									<p>
										※{t('editor.jr.mars.stationInfo.specialStationNameTip')}
										（例：高輪/ゲートウェイ→
									...
									<TicketFormLabel label={'2/3段式駅名種類'}>
										<PrettyInputRadioGroup
											value={drawParameters.station1Type.toString()}
											onChange={(value) => {
												setDrawParameters((prev) => ({
													...prev,
													station1Type: Number(value),
												}));
											}}
								...
								<div className="flex flex-col gap-[2px]">
									<TicketFormLabel label={'到着駅特定都区市内'}>
										<PrettyInputRadioGroup
											value={drawParameters.station2AreaChar}
											onChange={(value) => {
												setDrawParameters((prev) => ({
													...prev,
													station2AreaChar: value,
												}));
											}}
											list={TokuteiTokuShinai.map((TokuteiTokuShinaiItem) => {
												return {
									...
									<TicketFormLabel label={<span className="font-bold">到着駅</span>}>
										<div className="flex flex-wrap">
											<input
												className="border-[#79b8f2] border-2 font-bold"
								...
								<TicketFormLabel label={'経由'}>
									<input
										...
										onChange={(e) =>
											setDrawParameters((prev) => ({
												...prev,
												railways: e.target.value.split(/[,;|，、・·]/),
											}))
										}
									/>
								</TicketFormLabel>

								<TicketFormLabel label={'テキスト１'} className="border-t-[#ccc_!important]">
								...
								<TicketFormLabel label={'矢印様式'}>
									<PrettyInputRadioGroup
										value={drawParameters.isKaisukenArrow ? '1' : '0'}
										onChange={(value) => {
											setDrawParameters((prev) => ({
												...prev,
												isKaisukenArrow: value === '1',
											}));
										}}
										list={[
											{ value: '0', title: '→（一般）' },
							...
							<TitleContainer title="運行情報（仮）" className="flex flex-wrap gap-2">
								<TicketFormLabel label={'列車名'}>
									<input
										value={drawParameters.trainName}
										onChange={(e) =>
											setDrawParameters((prev) => ({
												...prev,
								...
								<TicketFormLabel label={'列車番'}>
									<div>
										<input
											value={drawParameters.trainNo}
											onChange={(e) =>
												setDrawParameters((prev) => ({
													...prev,
								...
								<TicketFormLabel label={'特急券有効期間（仮）'}>
									<div className="flex flex-wrap">
										<input
											type="date"
											value={drawParameters.date2}
								...
								<TicketFormLabel label={'乗車券有効期間（仮）'}>
									<div className="flex flex-wrap">
										<input
											type="date"
											value={drawParameters.date}
							...
							<TitleContainer title="購入情報（仮）" className="flex flex-wrap">
								<TicketFormLabel label={'購入手段（仮）'}>
									<PrettyInputRadioGroup
										...
										onChange={(e) =>
											setDrawParameters((prev) => ({
												...prev,
												paymentMethod: Number(e) as JRPaymentMethod,
											}))
										}
								...
								<Toggle
									...
									onChange={(value) => {
										setDrawParameters((prev) => ({
											...prev,
											isPaymentIssuingTheSamePlace: value,
										}));
									}}
								>
									発行発券場所は同じ
								</Toggle>
								<TicketFormLabel label={'発行場所'}>
									<div>
										<input
											className="w-full"
											value={drawParameters.paymentPlace}
											onChange={(e) =>
												setDrawParameters((prev) => ({
													...prev,
													paymentPlace: e.target.value,
												}))
											}
										/>
								...
								<Divider />
								<TicketFormLabel label={'発券場所'}>
									<input
										className=""
										value={drawParameters.issuingPlace}
										onChange={(e) =>
											setDrawParameters((prev) => ({
												...prev,
												issuingPlace: e.target.value,
											}))
										}
									/>
								</TicketFormLabel>
								<TicketFormLabel label={'発券日付'}>
									<input
										type="date"
								...
								<Divider />
								<TicketFormLabel label={'発券エリア番号'}>
									<div>
										<PrettyInputRadioGroup
											value={drawParameters.issuingAreaNo}
											onChange={(e) =>
												setDrawParameters((prev) => ({
													...prev,
													issuingAreaNo: e,
								...
								<TicketFormLabel label={'R通番'}>
									<input
										value={drawParameters.RCode}
										onChange={(e) =>
											setDrawParameters((prev) => ({
												...prev,
								...
								<TicketFormLabel label={'誤取消防止符号（C通番）'}>
									<input
										value={drawParameters.CCode}
										onChange={(e) =>
											setDrawParameters((prev) => ({
												...prev,
