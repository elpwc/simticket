...
export const TicketListViewerModal = ({ show, ticketInfo, onClose }: Props) => {
	const { t } = useLocale();
	const hint = useHint();
	...
	return (
		<Modal title={'閲覧'} isOpen={show} onClose={onClose} style={{ maxWidth: isMobile ? '100%' : '800px', width: isMobile ? '100%' : '80%' }} bodyStyle={{ padding: 0 }}>
			<div className="flex flex-col">
				<TicketViewer
					...
					ticketTypeId={ticketInfo === null ? 4 : ticketInfo.ticketTypeId}
					ticketData={ticketInfo === null ? CRWideTicketDrawParametersInitialValues : ticketInfo.ticketData}
					isFlip={isFlipSide}
				/>
