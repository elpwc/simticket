...
export const TicketListViewItem = ({ width, height, className, borderRadius, companyId, ticketTypeId, ticketData, onDelete }: Props) => {
  ...
  return (
    <div className={`relative inline-block overflow-hidden ${className ?? ''}`} style={{ width, height, borderRadius, border: 'solid 1px gray' }} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      ...
      <AnimatePresence>
        {showButtons && (
          ...
          >
            <button className="w-full text-[10px] px-2 py-1 rounded hover:bg-gray-100 transition">导出</button>
